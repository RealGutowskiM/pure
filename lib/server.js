...
function respond( stream, headers ) {
	...
	if ( headers[ ":path" ]?.startsWith( "/api/" ) )
		serve_backend( headers, stream );
	else
		serve_frontend( headers, stream );
}

async function serve_backend( headers, stream ) {
	try {
		const {
			response_headers,
			data
			...
			data_to_send = typeof data === "string" ?
				data :
				JSON.stringify( data );

		info( "Serving backend", { headers_to_send, data_to_send } );
		stream.respond( headers_to_send );
		stream.write( data_to_send );
	} catch ( err ) {
		error( "Serving backend failed", { error: err } );
...
function serve_frontend( headers, stream ) {
